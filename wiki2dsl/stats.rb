...
# == stats.rb ==
# Calculates some statistics about dictionaries extracted
# from Wikidata dumpfiles and saves them to an output file
# in the same directory
#
# Usage: Assuming you have a directory of compressed 
# wikidata dictionary files named [language-code].tar.gz, 
# navigate to the directory containing the tar.gz 
# compressed files directories and execute the following 
# command:
# 
# ruby stats.rb [language-code]
#
...
stat_file = File.open("wikidict-stats_#{dir}.txt", "w")
...
tar_extract.each do |entry|
  unless entry.directory? == true
    ...
    puts "The dictionary header looks like this:\n\n"
    puts header_stuff
    puts "Please wait. Calculating dictionary statistics..."
    hit_counter = 0
    dsl_name = dir + "/" + dict_name + "_wikidict.dsl"
    ...
    size = `ls -sh ../dsl/#{dsl_name}.dz`.gsub(/\s+\.\..*/, "")

    stat_line = dsl_name + "\t" + hit_counter.to_s + " entries, compressed size: #{size}"
    stats_arr.push(stat_line)
    size_line = hit_counter.to_s + "\t" + dict_name
    size_arr.push(size_line)
...
tar_extract.close
stats_arr.sort.each { |line| stat_file << line }
stat_file << "\n\nTop ten dictionaries by number of entries:\n"
size_arr.sort_by! { |x| x.to_i }
stat_file << size_arr.reverse[0..9].join("\n") + "\n"
stat_file.close
